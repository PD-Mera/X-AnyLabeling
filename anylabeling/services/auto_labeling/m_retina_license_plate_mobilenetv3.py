...
class RetinaLicensePlateMobilenetV3(Model):
    class Meta:
        ...
        if not model_abs_path or not os.path.isfile(model_abs_path):
            raise FileNotFoundError(
                QCoreApplication.translate(
                    "Model", "Could not download or initialize Retina License Plate MobilenetV3 model."
                )
            )
    ...
    def predict_shapes(self, image, image_path=None):
        ...
        for i, det in enumerate(det_results):
            ...
            x0 = int(np.maximum(np.minimum(det[0], float(self.retinanet.im_shape[1]) - 1.0), 0.0))
            y0 = int(np.maximum(np.minimum(det[1], float(self.retinanet.im_shape[0]) - 1.0), 0.0))
            x1 = int(np.maximum(np.minimum(det[2], float(self.retinanet.im_shape[1]) - 1.0), 0.0))
            y1 = int(np.maximum(np.minimum(det[3], float(self.retinanet.im_shape[0]) - 1.0), 0.0))
